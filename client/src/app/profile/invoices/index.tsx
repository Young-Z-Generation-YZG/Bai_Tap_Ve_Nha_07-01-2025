    ...
    <SafeAreaProvider>
      <SafeAreaView className='flex flex-1'>
        ...
        renderItem={({ item, index }: { item: string; index: number }) => (
          ...
          className={cn('mr-2 w-[200px] justify-center items-center rounded-xl', selectedStatus===item? 'bg-black': 'bg-gray-200' )}>
            <Text className={cn('font-TenorSans-Regular',selectedStatus===item? ' text-white': ' text-black' )}>{item}</Text>
          </TouchableOpacity>
        )}
        ...
        <ScrollView className='basis-[96%] px-5 pt-5 flex'>
          ...
          :
            (
              <View className='h-[600px] justify-center items-center gap-2' style={{flex:1}}>
                <Icons.InvoiceEmptyIcon className='w-10 h-10' />
                <Text className='font-Poppins-SemiBold text-[24px]'>No Orders Yet</Text>
              </View>
            )
          }
        </ScrollView>
