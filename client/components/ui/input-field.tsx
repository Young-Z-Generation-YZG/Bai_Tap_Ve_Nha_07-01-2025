      ...
      <Controller
         ...
         render={({ field: { onChange, onBlur, value } }) => {
            return (
               <View
                  className={cn(
                     'h-16 border rounded-2xl mt-10',
                     variant === 'outline' &&
                        'border-slate-200 border-t-0 border-l-0 border-r-0 border-b rounded-none',
                     className,
                  )}
               >
                  {Icon ? (
                     <View className="w-full h-full flex flex-row items-center pl-3">
                        ...
                        />
                        {required && (
                           <Text className="text-red-500 text-lg absolute right-3">
                              *
                           </Text>
                        )}
                        {type === 'password' && (
                     ...
                     <View
                        className={cn(
                           'h-full flex flex-row items-center justify-center px-3',
                           variant === 'outline' &&
                              'border-slate-200 border-t-0 border-l-0 border-r-0 border-b rounded-none',
                        )}
                     >
